...
**Typical values:** - Young forest: `min_radius = 0.05`, `max_radius
= 0.3` - Mature forest: `min_radius = 0.15`, `max_radius = 0.8` - Dense
forest: increase `min_votes` to 50-100 - Sparse forest: decrease
`min_votes` to 20-30
...
**Typical values:** - Urban areas: `min_volume = 50`, `min_votes = 200`
- Rural buildings: `min_volume = 10`, `min_votes = 100` - Small
structures: `min_volume = 1`, `min_votes = 50`
...
**Typical values:** - Ground detection: `distance_threshold = 0.1` -
Wall detection: `distance_threshold = 0.05` - Fine features:
...
**Typical values:** - Power lines: `min_votes = 50`, `dx = 0.1` -
Building edges: `min_votes = 20`, `dx = 0.05` - Fine wires: `min_votes
= 10`, `dx = 0.02`
...
**Too few detections:** - Decrease `min_votes` - Increase `max_radius`
(trees) or `max_planes` (buildings) - Check point cloud density - Verify
height normalization

**Too many false positives:** - Increase `min_votes` - Decrease
`max_radius` or increase `min_radius` - Add pre-filtering steps - Adjust
distance thresholds

**Poor accuracy:** - Decrease `dx` for finer resolution - Increase
`granularity` for more directions - Use more points (increase
`min_votes`) - Check for systematic errors in input data
