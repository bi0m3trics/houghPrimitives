...
The `detect_lines()` function uses RANSAC-based 3D line fitting with a
critical feature: **context-aware detection** via the `in_scene`
parameter.
...
**Use when:** Your point cloud contains **primarily linear features**
(cables, wires, infrastructure edges)

**Behavior:** Aggressive detection with relaxed parameters - Distance
threshold increased by 30% - Min votes reduced to 70% - Min length
reduced to 80% - Max iterations increased by 50%

**Example use cases:** - Power line corridor scans - Cable/wire
detection - Edge-only extractions

### Mode 2: Complex Scenes (`in_scene = TRUE`) \[DEFAULT\]

**Use when:** Lines coexist with **other shapes** (planes, cylinders,
buildings)

**Behavior:** Stricter detection using exact parameters - Only detects
standalone lines - Avoids detecting edges of planes as separate lines -
Use AFTER detecting other shapes

**Example use cases:** - Urban scenes with buildings and standalone
cables - Forestry with terrain and fallen logs - Industrial scans with
...
Maximum perpendicular distance from line for a point to be considered
part of that line.
...
**Impact:** - Too small: Miss valid line points, fragmented lines - Too
large: Include noise, merge nearby parallel lines
...
Number of RANSAC iterations. More iterations = more thorough search but
slower.
...
**Result:** Finds more lines, suitable when false positives are less
concerning than missing features.
...
**Key insight:** Detect planes/cylinders first, then use strict mode for
...
![Line detection results showing three detected lines in different
orientations.](line-detection-3d.png)
...
**Causes:** - Gaps in point coverage - `min_votes` too high relative to
gaps
...
**Causes:** - `distance_threshold` too large
...
## Comparison: in\_scene = TRUE vs FALSE
...
**Key Decision:** - **Line-only scenes**: Use `in_scene = FALSE` for
aggressive detection - **Complex scenes**: Use `in_scene = TRUE` after
detecting other shapes

**Critical workflow tip:** When detecting multiple shape types, always
detect lines **after planes** to avoid false positives from planar
edges.
