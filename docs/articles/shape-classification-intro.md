# Introduction to Shape Classification with houghPrimitives

## Overview

The `houghPrimitives` package provides comprehensive 3D shape detection
algorithms for point cloud data using Iterative Hough Transform methods.
This vignette introduces the key concepts and basic workflow for
classifying shapes in point clouds.

## Supported Shape Types

The package detects six types of 3D primitive
shapes:
...
Additionally, there’s a specialized function for forestry applications:
  ...
  - **Tree Boles**: `detect_tree_boles()` - Vertical cylinder with
...
Use `label_points = TRUE` to get back which points belong to which
shape:
...
For complex scenes with multiple shape types, detect shapes in order
...
Each detection function returns a list with:
  ...
  - **`n_*`**: Number of shapes detected (e.g., `n_spheres`,
    `n_cylinders`)
  - **Shape parameters**: Data frame with shape-specific attributes
  - **`point_labels`** (if requested): Integer vector assigning each
    point to a shape
...
### Geometric Constraints
  ...
  - **`min_radius`, `max_radius`**: Expected size range (spheres,
    cylinders)
  - **`min_length`, `min_height`**: Minimum extent (cylinders, lines)
  - **`distance_threshold`**: Maximum distance from shape for a point to
    be included

### Detection Sensitivity

  - **`min_votes`**: Minimum number of points required to form a valid
    shape
...
![Example multi-shape scene with cylinder, plane, and
sphere.](intro-shapes-3d.png)

Example multi-shape scene with cylinder, plane, and sphere.

## Next Steps

  - See `vignette("sphere-detection")` for detailed sphere detection
    examples
  - See `vignette("line-detection")` for line detection strategies
  - See `vignette("multi-shape-classification")` for comprehensive scene
    analysis
  - See `vignette("parameter-tuning")` for advanced parameter
    optimization
