# Sphere Detection in Point Clouds

## Introduction

Sphere detection is useful for identifying various spherical objects in
point clouds:
...
The `detect_spheres()` function uses spatial clustering combined with
geometric validation to identify spheres in 3D point clouds.

## How Sphere Detection Works

The algorithm follows these steps:

1.  **Spatial Clustering**: Groups nearby points using 3D grid-based
    clustering
2.  **Center Estimation**: Iteratively refines sphere center using
    geometric relationships
3.  **Radius Calculation**: Computes radius as mean distance from center
    to surface points
4.  **Validation**: Checks circularity, octant coverage, and radius
    consistency
5.  **Filtering**: Removes candidates that don’t meet geometric criteria

### Key Innovation: Iterative Center Refinement

Unlike simple centroid calculation, the algorithm uses the geometric
relationship:

\[\text{center} = \text{point} - \text{radius} \times \text{direction}\]

This iterative approach (3 iterations) correctly identifies the sphere
center even though surface points don’t average to the center.
...
**Tips:** - Set range based on expected object sizes in your data - Too
narrow: miss valid spheres - Too wide: longer processing time, more
false positives
...
**Impact:** - Higher `min_votes`: Fewer false positives, may miss
small/partial spheres - Lower `min_votes`: Detect more spheres, risk of
noise clusters
...
![3D visualization of sphere detection results. Detected spheres shown
with semi-transparent overlays.](sphere-detection-3d.png)
...
When spheres coexist with other shapes, detect them early in your
workflow:
...
**Possible causes:** - `min_votes` too high - Radius range
(`min_radius`, `max_radius`) doesn’t match data - Point density too low
...
**Possible causes:** - `min_votes` too low - Noise clusters being
detected as spheres
...
**Possible causes:** - Spheres too close together - Similar radii
causing clustering issues

**Solutions:** - Pre-segment point cloud by spatial regions - Detect
spheres in separate regions - Post-process results to merge/split as
needed

## Advanced: Partial Sphere Detection

The algorithm can detect partial spheres (e.g., hemisphere visible from
scanning position):
...
1.  **Know your data scale**: Set `min_radius` and `max_radius`
    appropriately
2.  **Start conservative**: Begin with higher `min_votes`, then relax if
    needed
3.  **Detect spheres early**: In multi-shape scenes, detect spheres
    before planes
4.  **Use point labels**: Enable `label_points = TRUE` for analysis and
    visualization
...
Key parameters: - `min_radius`, `max_radius`: Define size range -
`min_votes`: Control sensitivity/specificity tradeoff - `label_points`:
Get point-to-sphere assignments

For complex scenes with multiple shape types, detect spheres first, then
