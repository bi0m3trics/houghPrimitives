...
Added `in_scene` parameter to `detect_lines()` function to optimize detection based on context:
...
- Min votes reduced to 70% of specified value
- Min length reduced to 80% of specified value
...
**Use when**: Lines exist WITH other shapes (buildings, terrain, cylinders)

**Behavior**:
- Stricter detection
- Uses specified parameters as-is
- Only finds standalone lines not part of other structures
- Detects lines AFTER planes/cylinders have been identified
...
The `in_scene` parameter adjusts detection parameters internally in C++ code:
...
This ensures optimal detection for each use case without requiring users to manually tune many parameters.
