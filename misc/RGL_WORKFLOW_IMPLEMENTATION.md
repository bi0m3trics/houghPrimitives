...
A fully reproducible workflow for generating rgl 3D visualizations in pkgdown articles and vignettes has been implemented.
...
**Functions provided:**
- `setup_rgl_knitr()` - Configure headless rendering (CRITICAL - call in setup chunk)
- `save_rgl_snapshot()` - Save current rgl scene as PNG to man/figures/
- `create_rgl_widget()` - Generate interactive widget for HTML output
- `create_rgl_figure()` - High-level workflow function (automated open/save/close)

**Key features:**
- Headless rendering via `options(rgl.useNULL = TRUE)`
- Automatic directory creation
- pkgdown-safe default location (man/figures/)
...
- Reusable patterns (3 approaches)
- Complete shape detection visualization example
- File locations and pkgdown compatibility
...
1. Setup chunk (required in every vignette)
2. Simple static figure
3. Custom dimensions figure
4. Interactive widget with static fallback
5. Reusable scene function
6. Automated workflow
7. Shape detection visualization
8. Conditional widget (auto-detects HTML vs PDF)
9. Multiple scenes in sequence
10. Error handling
...
**Additions:**
- Enhanced setup chunk with `setup_rgl_knitr()`
- Complete 3D visualization example (~100 lines)
- Demonstrates detected spheres with semi-transparent overlays
- Shows both static PNG and interactive widget
- Includes inline comments explaining each step

### 5. Figure Storage
**Directory**: `man/figures/` (created)
**File**: `man/figures/README.md`
...
**Benefits:**
- ✓ Works in CRAN checks (no X11 required)
...
**Benefits:**
- ✓ `pkgdown::build_site()` finds figures automatically
...
**Benefits:**
- ✓ Rotation, zoom, pan with mouse
- ✓ Automatic fallback for PDF output
- ✓ Enhances user experience in HTML
...
```r
```{r my-figure, eval = requireNamespace("rgl", quietly = TRUE)}
library(rgl)
...
```r
```{r show-figure, echo = FALSE, out.width = "80%"}
knitr::include_graphics(fig_path)
```
```
...
```r
```{r widget, eval = knitr::is_html_output() && requireNamespace("rgl", quietly = TRUE)}
open3d()
spheres3d(0, 0, 0, radius = 1, color = "steelblue")
...
2. **save_rgl_snapshot()** (called for each figure):
...
3. **create_rgl_widget()** (optional, HTML only):
...
→ Use `man/figures/` directory
→ Rebuild site: `pkgdown::build_site()`

### Interactive widgets don't work
→ Check `knitr::is_html_output()`
